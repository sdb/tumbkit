...
var_mapping = {        
    ...
    ('Posts', 'Name') :                 lambda b, v, r: b.item['title'] if b.item.has_key('title') else b.item['url'], # TODO, {Name} is also  used in chat posts
    ('Posts', 'URL') :                  lambda b, v, r: b.item['url'],
    ('Posts', 'Target') :               lambda b, v, r: 'target="_blank"' if r.conf['open_links_in_new_window'] else '',
...
block_mapping = {
    ...
    ('Lines', 'Label') :            lambda b, p, r: p.item.has_key('label') and p.item['label'] != None and len(p.item['label']) > 0,
    ('Posts', 'Description') :      lambda b, p, r: p.item.has_key('description'),
